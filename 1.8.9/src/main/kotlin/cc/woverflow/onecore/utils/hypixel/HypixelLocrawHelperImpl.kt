package cc.woverflow.onecore.utils.hypixel

import cc.woverflow.onecore.api.OneCore
import cc.woverflow.onecore.api.utils.hypixel.HypixelGameType
import cc.woverflow.onecore.api.utils.hypixel.HypixelHelper
import cc.woverflow.onecore.api.utils.hypixel.HypixelLocraw
import cc.woverflow.onecore.api.utils.hypixel.HypixelLocrawHelper
...
class HypixelLocrawHelperImpl(
    ...
    @SubscribeEvent(priority = EventPriority.HIGH, receiveCanceled = true) fun onChatMessageReceived(event: ClientChatReceivedEvent) {
        ...
        val raw = OneCore.getJsonHelper().parse(stripped)
        if (!raw.isJsonObject) return
        val json = raw.asJsonObject
        val parsed = OneCore.getGson().fromJson(json, HypixelLocraw::class.java)
